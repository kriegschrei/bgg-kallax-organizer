...
const normaliseVersionList = (versionNode) => {
  ...
  const items = Array.isArray(versionNode.item) ? versionNode.item : [versionNode.item];
...
export const mapCollectionItems = (collectionJson = {}) => {
  ...
  const result = items.map((item) => {
    ...
    const versionEntries = normaliseVersionList(item.version);
    ...
    return {
      ...
      versions: versionEntries.length > 0 ? versionEntries : [{ versionId: -1 }],
    };
  });
